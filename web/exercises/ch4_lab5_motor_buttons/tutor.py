...
EXERCISE = {
    ...
    "objective": (
        ...
        "pressing btn_dir (GP13) toggles the forward flag — with both buttons using "
    ...
    "off_limits": [
        ...
        "the apply() helper function pattern — the student should try to factor it themselves",
    ...
    "hints": [
        ...
        "apply the motor — but you must check the current direction first.",
    ...
    "observation_checklist": [
        ...
        "Does the button fire repeatedly while held? The falling-edge condition is missing or wrong — check last_speed/last_dir updates.",
        "Does pressing btn_speed do nothing? Add print(s, last_speed) inside the loop to see the values changing.",
        "Does speed_idx go out of range? Check the modulo expression — it should be % len(SPEEDS), not a fixed number.",
        "Does direction change but speed reset to 0? Make sure you pass SPEEDS[speed_idx] not SPEEDS[0] when re-applying.",
        "Is there an IndexError? Print speed_idx and len(SPEEDS) — the modulo may not be wrapping correctly.",
    ],
...
- Guide the student to derive both conditions and the index update themselves.
  The connection to Chapter 1 button work is intentional — ask if they remember
...
showing s, last_speed, speed_idx, and forward on each iteration — that usually
...
"you've got the hardest part — now think about how to wrap the index."
...
the event-driven architecture used by the Connected Little Boxes framework —
