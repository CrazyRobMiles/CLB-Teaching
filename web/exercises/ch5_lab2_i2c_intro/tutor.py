...
# by writing directly to registers â€” no abstraction, no library.

EXERCISE = {
    "id": "ch5_lab2_i2c_intro",
    "phase": 4,
    ...
    "objective": (
        ...
        "writing ON/OFF counts directly to the channel 0 registers â€” all from the "
    ...
    "off_limits": [
        ...
        "the ON/OFF count values for 0Â°, 90Â°, 180Â° before the student has attempted the pulse-width-to-count conversion",
    ],
    ...
    "hints": [
        ...
        "You need to write to it to wake the chip. The description shows the exact byte â€” "
        ...
        "write this register â€” what does the description say about that?",
        ...
        "to a 1.5 ms pulse (90Â°)?",

        # Hint 5
        "1.5 ms / 20 ms * 4096 = 307.2 â†’ 307 counts. In hex: 0x133, so OFF_L = 0x33, "
        "OFF_H = 0x01. The writeto_mem call needs 4 bytes: [ON_L, ON_H, OFF_L, OFF_H]. "
        "Try it â€” does the servo move to centre?",
    ],
    ...
    "success_indicators": [
        ...
        "writing [0, 0, 0x77, 0x01] to register 0x06 moves the servo to approximately 90Â°",
    ...
    "observation_checklist": [
        "Does i2c.scan() return anything? If empty, check SDAâ†’GP0 and SCLâ†’GP1 are not swapped.",
        "Does the PCA9685 board have power? Check VCCâ†’3V3 and GNDâ†’GND connections.",
        "After waking the chip, read back register 0x00 â€” does it show 0x20?",
...
console exploration exercise â€” there is no file to edit. The student types
...
- Help the student reason through the prescale formula and the pulse-count
  arithmetic â€” guide them to the answer rather than stating it.
...
The register-level work can feel tedious â€” acknowledge this honestly and
...
library for I2C devices â€” in MicroPython, CircuitPython, Arduino, Linux
kernel â€” is doing exactly this at its core. They have seen through the
