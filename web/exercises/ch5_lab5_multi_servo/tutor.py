...
EXERCISE = {
    ...
    "objective": (
        ...
        "and btn_ccw/btn_cw nudge the active servo by ±5°, with the selected servo "
    ...
    "hints": [
        ...
        "already clamps to 0–180. How can you combine these two facts into a single "
        "call that handles everything?",

        # Hint 2
        "nudge() is literally: self.angle(self._degrees + delta). Because angle() clamps, "
        "nudging past 0 or 180 is safe. Now look at the main loop — what does a 'falling "
        ...
        "LOW means pressed. Write the condition for btn_prev (index 0) and test it — "
        ...
        "Python's % operator handles this cleanly — (active + 1) % len(servos).",
    ...
    "success_indicators": [
        ...
        "btn_cw moves only the currently active servo by +5° per press",
        "btn_ccw moves only the currently active servo by -5° per press",
        "nudging past 0° or 180° stops at the limit — no out-of-range movement",
    ...
    "observation_checklist": [
        ...
        "Does pressing a button trigger multiple actions? The falling-edge condition (last==1 and btns==0) should fire only once per press — check that last = btns is inside the loop, after all checks.",
        "Does active wrap correctly? Test: from servo 3, press btn_next — does it go to 0?",
        "Is nudge() implemented? If it is still 'pass', btn_cw and btn_ccw will silently do nothing.",
        "Are the buttons wired to the correct pins — GP14, GP13, GP12, GP11 — with PULL_UP?",
...
created at 90°, and four buttons set up with PULL_UP. The student must implement:

1. ServoDriver.nudge(delta) — a one-liner that calls self.angle().
2. Four falling-edge detection blocks in the while loop, each taking an action
   (cycle active servo selection, or nudge the active servo by ±5°).
...
student has built — one I2C bus, a hardware PWM chip managing four independent
...
sketch in the description — this program is one step away from a real
