...
EXERCISE = {
    ...
    "objective": (
        ...
        "clamp the input to 0–180 degrees, map it linearly to a pulse count "
        "between MIN_PULSE (150) and MAX_PULSE (600), and call set_pwm() on "
        "the PCA9685 object — resulting in a clean servo.angle(90) interface "
    ...
    "hints": [
        ...
        "The description lists them. Start with clamping — what Python built-ins "
        "clamp a value to a range without an if/else?",

        # Hint 2
        "max(0, min(180, degrees)) clamps correctly. Now think about the mapping. "
        "At 0° you want pulse count 150; at 180° you want 600. Write the "
        "relationship as a formula — what is the pulse at an arbitrary degrees value?",

        # Hint 3
        "The linear map is: pulse = MIN_PULSE + (MAX_PULSE - MIN_PULSE) * degrees / 180. "
        "At 0°: 150 + 450 * 0/180 = 150. At 90°: 150 + 450 * 90/180 = 375. "
        "At 180°: 150 + 450 = 600. The result is a float — what does set_pwm expect, "
        ...
        "That is the complete angle() — three lines. Save and run.",
    ],
    ...
    "success_indicators": [
        ...
        "servo0.angle(200) does not overshoot — clamping is working",
    ...
    "observation_checklist": [
        ...
        "Is the PCA9685 wired correctly — SDA→GP0, SCL→GP1, VCC→3V3, GND→GND?",
        "Is the servo powered from 5V (VBUS), not 3.3V?",
        "If angle() raises a TypeError, check whether you are passing a float to set_pwm — add int().",
        "If the servo reaches only part of its range, try adjusting MIN_PULSE or MAX_PULSE by 10–20 counts.",
        "If the servo grinds at an end-stop, the pulse is too wide or narrow — back off the corresponding limit.",
    ],
...
class is fully implemented and working — the student's only task is to fill in
angle() with three steps: clamp, map, call.

The deeper goal is encapsulation: the student has spent Labs 2–3 writing raw
...
- Guide the student to derive the linear mapping formula themselves. The
  concept of linear interpolation between two endpoints is important — do
...
point out that the raw register values and prescale formulas from Labs 2–3 have
...
This is what encapsulation achieves — and it is exactly the same design pattern
