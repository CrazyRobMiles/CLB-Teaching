...
btn_prev = Pin(14, Pin.IN, Pin.PULL_UP)
btn_next = Pin(13, Pin.IN, Pin.PULL_UP)
btn_ccw  = Pin(12, Pin.IN, Pin.PULL_UP)
btn_cw   = Pin(11, Pin.IN, Pin.PULL_UP)
...
while True:
    ...
    if last[0] == 1 and btns[0] == 0:
        active = (active - 1) % len(servos)
        print(f"Servo {active}: {servos[active]._degrees}°")

    if last[1] == 1 and btns[1] == 0:
        active = (active + 1) % len(servos)
        print(f"Servo {active}: {servos[active]._degrees}°")

    if last[2] == 1 and btns[2] == 0:
        servos[active].nudge(-5)
        print(f"Servo {active}: {servos[active]._degrees}°")

    if last[3] == 1 and btns[3] == 0:
        servos[active].nudge(5)
        print(f"Servo {active}: {servos[active]._degrees}°")
